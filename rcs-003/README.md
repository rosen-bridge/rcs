...
4. Implement unit tests for all functions of the network class. Note that no real request should be sent in the tests and the connector should be completely mocked.
    - for tests, refer to [`BitcoinRpcNetwork` tests](https://github.com/rosen-bridge/scanner/blob/1b37e135e6f9f82d92cea0390a5631e384672713/packages/scanners/bitcoin-scanner/tests/network/bitcoinRpcNetwork.spec.ts)
    - mocking depends on the network connector. For mocking `axios` refer to [`axios.mock.ts`](https://github.com/rosen-bridge/scanner/blob/1b37e135e6f9f82d92cea0390a5631e384672713/packages/scanners/bitcoin-scanner/tests/mocked/axiosRpc.mock.ts) in the `bitcoin-rpc-scanner` tests. For mocking classes (e.g., `ethers.JsonRpcProvider`) refer to [`JsonRpcProvider.mock.ts`](https://github.com/rosen-bridge/scanner/blob/1b37e135e6f9f82d92cea0390a5631e384672713/packages/scanners/evm-scanner/tests/mocked/jsonRpcProvider.mock.ts) in the `evm-rpc-scanner` tests.
...
1. Add the new chain name to [`const.ts` file](https://github.com/rosen-bridge/utils/blob/6315a255314976fb055db3578a2d6d631882142a/packages/address-codec/lib/const.ts).
...
#### EVM Rosen Extractor
The new chain should be added to the `SUPPORTED_CHAINS` list in the [`const.ts` file](https://github.com/rosen-bridge/utils/blob/6315a255314976fb055db3578a2d6d631882142a/packages/rosen-extractor/lib/getRosenData/const.ts).

#### EVM Observation Extractor
A package is already exist for EVM as `@rosen-bridge/evm-observation-extractor` in the [Scanner repository](https://github.com/rosen-bridge/scanner). The extractor class should be implemented while inheriting from the `EvmRpcObservationExtractor ` class (refer to the [`BinanceRpcObservationExtractor ` implementation](https://github.com/rosen-bridge/scanner/commit/6efd277e5f4a48b2f791c0a4a8546ed4fb21db6d) for example).
